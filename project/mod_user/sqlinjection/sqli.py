...
class SqlInjection:
    def __init__(self):
        ...
        self.payloads = open('project/mod_user/sqlinjection/payloads.txt', 'r').read().splitlines()
        self.error_messages = open('project/mod_user/sqlinjection/errors.txt', 'r').read().splitlines()
    ...
    def run(self, url, base_url):
        ...
        for index, form in enumerate(forms):
            for payload in self.payloads:
                sleep(0.01)
                injected_payload = self.inject_payload(url, base_url, form, payload)
                if self.has_error(injected_payload):
